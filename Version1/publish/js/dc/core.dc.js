...
BLTdc.inti = function(){
  if (typeof(Enabler) !== "undefined") {
    if (Enabler.isInitialized()) {
      ...
      Enabler.addEventListener(studio.events.StudioEvent.INIT, this.politeInit);
    }
  } else {
...
BLTdc.politeInit = function () {
  if (Enabler.isPageLoaded()) {
    this.startAd();
  } else {
    Enabler.addEventListener(studio.events.StudioEvent.PAGE_LOADED, BLTdc.startAd);
  }
};

BLTdc.startAd = function(){
  Enabler.addEventListener(studio.events.StudioEvent.EXIT, BLTdc.onExit);
  document.dispatchEvent(new CustomEvent(BLTdc.events.CORE_READY));
  //BLTdc.initCore();
};


BLTdc.onExit = function () {
  BLTdc.dispatchEvent(BLTdc.events.EXIT);
};


BLTdc.dispatchEvent = function (e, args) {
  if (BLTdc.eventslist[e]) {
    BLTdc.eventslist[e](args);
  }
};


BLTdc.addEventListener = function (e, callback) {
  BLTdc.eventslist = BLTdc.eventslist || {};
  BLTdc.eventslist[e] = callback;
};
...
// Load Ad
window.addEventListener("load", BLTdc.init);
